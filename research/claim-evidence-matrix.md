...
ingestion, source-map binding, quarantine, and approval-gated publication.
...
| C2 | A working CCS-2.1 research prototype that realizes the formal obligations | M2 canonical models and Schema documents; package loader; filesystem CAS; SQLite Registry; M3 source-grounded builder and quarantine; later M4-M6 resolver, compiler, validator, swap controller, and adapters | 218 M2 unit/property tests, 28 M3 integration/security tests, plus M1 formal regressions; later G1 vertical slice | Canonical digest determinism/sensitivity; derived-artifact isolation; load integrity; immutable publication; source-map fidelity; trust-promotion fail-closed behavior; unauthorized/tampered-read rejection; local Schema/Python assertion parity; later closure, traceability, pointer update, rollback | Sections 4-5 | Seven-module strict model; explicit JCS identity; fail-closed package loader; immutable local CAS/Registry; local Schema parity; deterministic source snapshots and structural atomization; Git/CAS/external evidence bindings; approval-gated M2-backed build/load/publish path; real Red-Green history | M4-M6 production path, behavioral validator, active pointer, adapters, and a real build-to-rollback slice | Authors approved the M2 field mapping, remediation, and disclosed generic JSON Schema semantic-pass boundary at `eec2c1f`; M3 evidence remains engineering-only and does not constitute the complete prototype |
...
M2-backed public build/load/publish evidence. The focused M3 suite has 28
passing tests and is included in the 280-test cumulative run. These are
engineering tests, not benchmark observations or empirical support for TER,
PIP, BSR, C2-C5, baseline advantage, or cross-Agent generality. The M3 human
